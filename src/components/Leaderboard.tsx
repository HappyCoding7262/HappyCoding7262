...
export default function Leaderboard({ users, tasks = [], onSendHeart, currentUser, activeStaffName }: LeaderboardProps) {
  const [timeframe, setTimeframe] = useState<Timeframe>('month');
  const [scoreType, setScoreType] = useState<'groep' | 'leidster'>('groep');
  ...
  const individualLeaderboardData = useMemo(() => {
    ...
    users.forEach(user => {
      if (user.role === 'Leidster' && user.staffNames) {
        const names = user.staffNames.split(',').map(s => s.trim()).filter(Boolean);
        names.forEach(name => {
          // Tasks completed by this specific person
          const personCompletedTasks = tasks.filter(t => 
            t.status === 'Completed' && 
            t.completedByName === name && 
            t.completedByUserId === user.id
          );

          const personCompletedTasksThisMonth = personCompletedTasks.filter(t => 
            t.completedAt && 
            new Date(t.completedAt).getMonth() === currentMonth &&
            new Date(t.completedAt).getFullYear() === currentYear
          );

          let points = user.staffPoints?.[name] || 0;
          if (timeframe === 'month') {
            points = personCompletedTasksThisMonth.length * 15;
          }
          ...
          list.push({
            ...
            tasksCompletedPeriod: personCompletedTasksThisMonth.length,
            tasksCompletedAllTime: personCompletedTasks.length,
            avatar: '👩‍🏫',
            hearts: 0,
            groupId: user.id,
    ...
    <div className="bg-white border border-brand-border rounded-[32px] p-6 shadow-sm">
      <div className="mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          {/* Rank Selector: Groepen / Klassen vs Leidsters */}
          <div className="flex bg-brand-bg rounded-full p-1 border border-brand-border">
            <button 
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${scoreType === 'groep' ? 'bg-white text-brand-gray-dark shadow-sm' : 'text-brand-gray hover:text-brand-gray-dark'}`}
            >
              🏢 Groepen
            </button>
            <button 
              onClick={() => {
                setScoreType('leidster');
                setExpandedUserId(null);
              }}
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${scoreType === 'leidster' ? 'bg-white text-brand-gray-dark shadow-sm' : 'text-brand-gray hover:text-brand-gray-dark'}`}
            >
              👩‍🏫 Leidsters
            </button>
          </div>
          ...
          <div className="flex bg-brand-bg rounded-full p-1 border border-brand-border">
            <button 
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${timeframe === 'month' ? 'bg-white text-brand-gray-dark shadow-sm' : 'text-brand-gray hover:text-brand-gray-dark'}`}
            >
              Deze Maand
            </button>
            <button 
              onClick={() => setTimeframe('all-time')}
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${timeframe === 'all-time' ? 'bg-white text-brand-gray-dark shadow-sm' : 'text-brand-gray hover:text-brand-gray-dark'}`}
            >
              All-time
            </button>
          </div>
      ...
      <div className="space-y-4">
        {scoreType === 'groep' ? (
          ...
          individualLeaderboardData.map((person, idx) => {
            const isTop3 = idx < 3 && person.displayPoints > 0;
            const isMe = person.groupId === currentUser.id && activeStaffName === person.name;
            const isExpanded = expandedUserId === `${person.groupId}-${person.name}`;

            const personTasks = tasks.filter(t => 
              t.status === 'Completed' && 
              t.completedByName === person.name &&
              t.completedByUserId === person.groupId
            );

            const periodTasks = personTasks.filter(t => {
              if (timeframe === 'all-time') return true;
              if (!t.completedAt) return false;
              const now = new Date();
              const taskDate = new Date(t.completedAt);
            ...
            return (
              <div 
                key={`${person.groupId}-${person.name}`} 
                className={`rounded-[24px] border transition-all duration-300 overflow-hidden flex flex-col ${
                  isMe ? 'border-brand-olive bg-brand-sage-light/50 ring-2 ring-brand-olive ring-offset-2' :
                  isTop3 ? 'border-brand-peach bg-brand-peach-light/30' : 'border-brand-border bg-brand-bg'
                }`}
              >
                <div 
                  onClick={() => setExpandedUserId(isExpanded ? null : `${person.groupId}-${person.name}`)}
                  className="flex items-center justify-between p-4 cursor-pointer hover:bg-black/[0.01] active:opacity-90 select-none"
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      setExpandedUserId(isExpanded ? null : `${person.groupId}-${person.name}`);
                    }
                  }}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-12 h-12 rounded-full border-2 border-white shadow-sm flex items-center justify-center text-xl bg-brand-bg relative animate-fade-in shrink-0">
                      👩‍🏫
                      {isTop3 && (
                        <div className="absolute -top-2 -right-2">
                           {idx === 0 && <span className="text-lg">👑</span>}
                    ...
                    <div className="text-left min-w-0">
                      <h3 className="font-semibold text-brand-gray-dark flex items-center gap-1.5 flex-wrap">
                        <span className="truncate">{person.name}</span>
                        <span className="text-[10px] text-brand-gray-light font-normal">({person.groupName})</span>
                        {isMe && <span className="text-[10px] bg-brand-sage-light text-brand-olive px-2 py-0.5 rounded-full font-bold">Jij</span>}
                      </h3>
                      <p className="text-xs text-brand-gray-light">
                        {person.displayPoints} {person.displayPoints === 1 ? 'punt' : 'punten'}
                        {timeframe === 'all-time' && ` • ${person.tasksCompletedAllTime} ${person.tasksCompletedAllTime === 1 ? 'taak' : 'taken'} • individueel`}
                        {timeframe === 'month' && ` • ${person.tasksCompletedPeriod} ${person.tasksCompletedPeriod === 1 ? 'taak' : 'taken'} voltooid`}
                      </p>
                    </div>
                  </div>
